...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!project) return { title: "Проект не найден — ELSYSTAR", robots: { index: false, follow: false } };
  return buildEntityMetadata({
    path: `/projects/${project.slug}`,
    title: project.seoTitle || `${project.title} — ELSYSTAR`,
    description: project.seoDescription || project.summary,
    image: project.coverImageUrl,
    index: !project.isDemo,
    type: "article",
  });
}
...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const project = await getProjectBySlug(slug);
  if (!project) notFound();
  const jsonLd = !project.isDemo ? projectJsonLd(project) : null;
  ...
  return (
    <main>
      ...
      {project.coverImageUrl ? <section className="caseHeroMedia shell"><img src={project.coverImageUrl} alt={project.city ? `Транспортная инфраструктура — ${project.city}` : project.title} /><div className="caseHeroContent"><a className="backLink" href="/projects">← Все проекты</a>{project.isDemo && <span className="demoBadge">Демонстрационный проект</span>}<p className="eyebrow">{[project.city, project.region, project.year].filter(Boolean).join(" · ") || "ПРОЕКТ ELSYSTAR"}</p><h1>{project.title}</h1><p className="lead">{project.summary}</p>{project.metrics.length > 0 && <div className="projectMetrics">{project.metrics.map((metric)=><div key={`${metric.value}-${metric.label}`}><strong>{metric.value}</strong><span>{metric.label}</span></div>)}</div>}</div></section> : <section className="solutionHero shell"><a className="backLink" href="/projects">← Все проекты</a>{project.isDemo && <span className="demoBadge">Демонстрационный проект</span>}<p className="eyebrow">{[project.city, project.region, project.year].filter(Boolean).join(" · ") || "ПРОЕКТ ELSYSTAR"}</p><h1>{project.title}</h1><p className="lead">{project.summary}</p>{project.metrics.length > 0 && <div className="projectMetrics">{project.metrics.map((metric)=><div key={`${metric.value}-${metric.label}`}><strong>{metric.value}</strong><span>{metric.label}</span></div>)}</div>}</section>}
