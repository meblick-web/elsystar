  ...
  return (
    <main>
      <header className="header shell">
        ...
        <div className="actions"><span>RU / EN</span><a className="button small" data-analytics="cta_click" data-product-id={product.id} href="#request">Получить КП</a></div>
      </header>
      ...
      <section className="productHero shell">
        <div>
          ...
          <div className="heroButtons"><a className="button" data-analytics="cta_click" data-product-id={product.id} href="#request">Запросить КП</a>{product.documents.length > 0 && <a className="button ghost" href="#documents">Документация</a>}</div>
        </div>
      ...
      <section id="documents" className="documentsSection shell">
        <div className="sectionHead"><div><p className="eyebrow">ДОКУМЕНТАЦИЯ</p><h2>Материалы по продукту</h2></div><a href="/support">Весь центр документации →</a></div>
      ...
      <section id="request" className="requestSection shell">
        <div className="requestIntro"><p className="eyebrow">КОНСУЛЬТАЦИЯ</p><h2>Обсудить применение {product.model}</h2><p>Поможем проверить совместимость, подобрать конфигурацию и подготовить коммерческое предложение.</p></div>
        <LeadForm productId={product.id} productLabel={product.model} />
      </section>

      <footer className="footer"><div className="shell footerInner"><div><div className="logo light">ELSY<span>STAR</span></div><p>Интеллектуальные решения для управления движением.</p></div><div><b>Продукция</b><a href="/products">Контроллеры</a><a href="/#megapolis">Мегаполис</a><a href="/support">Документация</a></div><div><b>Компания</b><a href="/#company">О компании</a><a href="/#contacts">Контакты</a></div><div><b>Связаться</b><a href="tel:+79674232054">+7 (967) 423-20-54</a><a href="mailto:arkhast@mail.ru">arkhast@mail.ru</a></div></div></footer>
    </main>
  );
}
