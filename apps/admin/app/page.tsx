...
async function getDashboardData() {
  ...
  try {
    const [events, products, drafts, archived, leads, recentLeads] = await Promise.all([
      prisma.analyticsEvent.findMany({
        where: { createdAt: { gte: since } },
        select: { type: true, visitorId: true, path: true, createdAt: true },
        take: 20000,
      }),
  ...
  return (
    <div className="admin">
      <aside>
        ...
        <nav>
          <a className="active" href="/">Обзор</a>
      ...
      <main>
        ...
        <section className="metrics" id="analytics">
          {data.metrics.map((metric) => <article key={metric[0]}><span>{metric[0]}</span><strong>{metric[1]}</strong><small>{data.configured ? "за последние 30 дней" : "ожидает подключения БД"}</small></article>)}
        </section>

        <section className="grid">
          <article className="chartCard"><div className="title"><h2>Просмотры страниц</h2><span>14 дней</span></div><div className="bars">{data.chart.map((value,index) => <i key={index} title={`${value} просмотров`} style={{height:`${Math.max(5, Math.round((value / maxValue) * 100))}%`}} />)}</div></article>
          <article><div className="title"><h2>Популярные страницы</h2><span>30 дней</span></div>{data.popularPages.length ? <div className="list">{data.popularPages.map((page,index) => <div key={page[0]}><span><b>{index+1}</b>{page[0]}</span><strong>{page[1]}</strong></div>)}</div> : <p className="empty">Данные появятся после первых посещений публичного сайта.</p>}</article>
        </section>
