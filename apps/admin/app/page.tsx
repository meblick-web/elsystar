...
async function getDashboardData() {
  ...
  const empty = { configured, metrics: [["Посетители", "0"], ["Просмотры", "0"], ["Заявки КП", "0"], ["Скачивания", "0"]], popularPages: [] as Array<[string, string]>, chart: Array.from({ length: 14 }, () => 0), catalog: { products: 0, drafts: 0, archived: 0 }, content: { solutions: 0, projects: 0 }, recentLeads: [] as Array<{ id: string; name: string; company: string | null; createdAt: Date }> };
  ...
  try {
    const [events, products, drafts, archived, leads, recentLeads, solutions, projects] = await Promise.all([
      ...
      prisma.lead.count({ where: { createdAt: { gte: since } } }), prisma.lead.findMany({ orderBy: { createdAt: "desc" }, take: 5, select: { id: true, name: true, company: true, createdAt: true } }), prisma.solution.count(), prisma.project.count(),
    ]);
...
export default async function Admin() {
  ...
  return <div className="admin"><aside><div className="brand">ELSY<span>STAR</span><small>ADMIN</small></div><nav><a className="active" href="/">Обзор</a><a href="/analytics">Аналитика</a><a href="/homepage">Главная</a><a href="/products">Продукция</a><a href="/solutions">Решения</a><a href="/projects">Проекты</a><a href="/documents">Документация</a><a href="/leads">Заявки</a><a href="/media">Медиа</a><a href="/seo">SEO</a><a href="/users">Пользователи</a><a href="/audit">Журнал действий</a></nav></aside><main>
    ...
    <section className="grid lower"><article><div className="title"><h2>Каталог</h2><a href="/products">Управлять →</a></div><div className="catalog"><div><strong>{data.catalog.products}</strong><span>продуктов</span></div><div><strong>{data.catalog.drafts}</strong><span>черновиков</span></div><div><strong>{data.catalog.archived}</strong><span>архивных</span></div></div></article><article><div className="title"><h2>Контент</h2><span>CMS</span></div><div className="catalog"><div><strong>{data.content.solutions}</strong><span>решений</span></div><div><strong>{data.content.projects}</strong><span>проектов</span></div></div></article></section>
    <section className="quickModules"><a href="/analytics"><strong>Аналитика</strong><span>Источники, устройства и конверсии →</span></a><a href="/seo"><strong>SEO</strong><span>Metadata и redirects →</span></a><a href="/users"><strong>Пользователи</strong><span>Роли и доступ →</span></a><a href="/audit"><strong>Журнал действий</strong><span>История административных операций →</span></a><a href="/leads"><strong>Заявки</strong><span>Коммерческие обращения →</span></a></section>
    <footer>v0.1.0-alpha.5 · Analytics, Users, Audit & SEO Operations</footer>
  </main></div>;
}
