...
async function getDashboardData() {
  ...
  const empty = { configured, metrics: [["Посетители", "0"], ["Просмотры", "0"], ["Заявки КП", "0"], ["Скачивания", "0"]], popularPages: [] as Array<[string, string]>, chart: Array.from({ length: 14 }, () => 0), catalog: { products: 0, drafts: 0, archived: 0 }, content: { solutions: 0, projects: 0, faq: 0, translations: 0 }, recentLeads: [] as Array<{ id: string; name: string; company: string | null; createdAt: Date }> };
  if (!configured || !prisma) return empty;
  const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  const chartSince = new Date(Date.now() - 13 * 24 * 60 * 60 * 1000);
  try {
    const [events, products, drafts, archived, leads, recentLeads, solutions, projects, faq, translations] = await Promise.all([
      ...
      prisma.lead.count({ where: { createdAt: { gte: since } } }), prisma.lead.findMany({ orderBy: { createdAt: "desc" }, take: 5, select: { id: true, name: true, company: true, createdAt: true } }), prisma.solution.count(), prisma.project.count(), prisma.faqEntry.count(), prisma.contentTranslation.count({ where: { locale: "en" } }),
    ]);
    ...
    return { configured: true, metrics: [["Посетители", String(visitors)], ["Просмотры", String(pageViews.length)], ["Заявки КП", String(leads)], ["Скачивания", String(downloads)]], popularPages, chart, catalog: { products, drafts, archived }, content: { solutions, projects, faq, translations }, recentLeads };
